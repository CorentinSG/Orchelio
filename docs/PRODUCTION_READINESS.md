...
| Separation of duties: the requester may not decide | 🟡 | Offered per firm and enforced inside `decideApproval`, which is the only function that records a decision. The requester is named on every card and in every audit entry whether the firm enforces it or not. **Default off**, because a sole practitioner refused unconditionally has an undecidable queue — see ADR-0019. Switching it on is refused while the firm has fewer than two people who may decide. |
...
| A superseded analysis does not leave its approval pending | ✅ | Re-running an analysis retires the request the previous one left waiting. `"superseded"` is its own status, kept apart from the four decisions by three predicates that partition the vocabulary — a unit test fails if a status belongs to none or more than one, which is what stops a future value being quietly counted as a decision. The row is kept, says that nobody decided it, cannot then be decided, and is logged under `approval.superseded` rather than `approval.decided`. See ADR-0020. |
