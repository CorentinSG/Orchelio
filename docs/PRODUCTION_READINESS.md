...
| A superseded analysis does not leave its approval pending | ⛔ | Re-running an analysis raises a new request and leaves the previous one waiting for ever. A matter in the demonstration database has accumulated 113 pending "rely on an AI analysis" requests this way, which buries the one that matters. Fixing it properly needs a status for *superseded* — a fifth value that is not a decision — and that touches the invariant that there are exactly four decisions, so it is named here rather than patched. The screens now report the true count and say how many they are showing. |
